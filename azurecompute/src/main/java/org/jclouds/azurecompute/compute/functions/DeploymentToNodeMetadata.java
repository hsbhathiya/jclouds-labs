...
public class DeploymentToNodeMetadata implements Function<Deployment, NodeMetadata> {
	...
	@Inject DeploymentToNodeMetadata(@Memoized Supplier<Set<? extends Location>> locations,
	...
	private String getHostname(Deployment from) {
		if (from.roleInstanceList() != null && !from.roleInstanceList().isEmpty()) {
			return from.roleInstanceList().get(0).hostname();
		}
		return from.name();
	}
