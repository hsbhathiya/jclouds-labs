...
public class DeploymentApiMockTest extends BaseAzureComputeApiMockTest {
   ...
   public void testCreateLinux() throws Exception {
      ...
      try {
         ...
         OSVirtualHardDiskParam osParam = OSVirtualHardDiskParam.builder().sourceImageName(OSImage.name())
               .mediaLink(OSImage.mediaLink()).os(org.jclouds.azurecompute.domain.OSImage.Type.LINUX)
               .diskName("myinstance-osdisk").diskLabel("myinstance-osdisk").hostCaching("ReadWrite").build();

         RoleParam roleParam = RoleParam.builder().roleName("Myinstance2").roleSize(RoleSize.Type.MEDIUM)
               .osVirtualHardDiskParam(osParam).linuxConfigurationSet(linuxConfig).build();

         DeploymentParams deploymentParams = DeploymentParams.builder().name("mydeployment")
               .reservedIpName("myreservedip")
               .externalEndpoint(DeploymentParams.ExternalEndpoint.inboundTcpToLocalPort(80, 8080))
               .externalEndpoint(DeploymentParams.ExternalEndpoint.inboundUdpToLocalPort(53, 53)).roleParam(roleParam)
               .build();
   ...
   public void testCreateWindows() throws Exception {
      ...
      try {
         ...
         WindowsConfigurationSetParams winConfig = WindowsConfigurationSetParams.builder().adminUserName("testAzure2")
               .adminPassword("azure@jclouds2").build();

         OSVirtualHardDiskParam osParam = OSVirtualHardDiskParam.builder().sourceImageName(OSImage.name())
               .mediaLink(OSImage.mediaLink()).os(org.jclouds.azurecompute.domain.OSImage.Type.WINDOWS)
               .diskName("myinstance-osdisk").diskLabel("myinstance-osdisk").hostCaching("ReadWrite").build();

         RoleParam roleParam = RoleParam.builder().roleName("Myinstance2").roleSize(RoleSize.Type.MEDIUM)
               .osVirtualHardDiskParam(osParam).windowsConfigurationSet(winConfig).build();
         ...
         assertEquals(api.create(deploymentParams), "request-1");
         assertSent(server, "POST", "/services/hostedservices/myservice/deployments",
               "/newdeploymentparams-windows.xml");
      } finally {
         server.shutdown();
      }
   }
