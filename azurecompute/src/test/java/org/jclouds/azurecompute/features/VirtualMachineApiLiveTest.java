...
public class VirtualMachineApiLiveTest
      extends BaseAzureComputeApiLiveTest {

   private static final String CLOUD_SERVICE = String
         .format("%s%d-%s", System.getProperty("user.name"), RAND, VirtualMachineApiLiveTest.class.getSimpleName())
         .toLowerCase();
   private static final String DEPLOYMENT = String
         .format("%s%d-%s", System.getProperty("user.name"), RAND, VirtualMachineApiLiveTest.class.getSimpleName())
         .toLowerCase();
   ...
   public void setup() {
      ...
      LinuxConfigurationSetParams linuxConfig = LinuxConfigurationSetParams.builder().hostName("bhash90.jclouds.azure")
            .userName("test").userPassword("supersecurePassword1!").build();

      roleName = DEPLOYMENT;
      String diskName = roleName + "osdisk" + (int) (Math.random() * 100);
      OSVirtualHardDiskParam osParam = OSVirtualHardDiskParam.builder().sourceImageName(IMAGE_NAME)
            .mediaLink(AzureComputeServiceAdapter.createMediaLink(storageService.serviceName(), DEPLOYMENT))
            .os(org.jclouds.azurecompute.domain.OSImage.Type.LINUX).diskName(diskName).diskLabel(diskName)
            .hostCaching("ReadWrite").build();

      RoleParam roleParam = RoleParam.builder().roleName(roleName).roleSize(RoleSize.Type.BASIC_A2)
            .osVirtualHardDiskParam(osParam).linuxConfigurationSet(linuxConfig).build();

      DeploymentParams params = DeploymentParams.builder().name(DEPLOYMENT).roleParam(roleParam)
            .virtualNetworkName(virtualNetworkSite.name())
            .externalEndpoint(DeploymentParams.ExternalEndpoint.inboundTcpToLocalPort(22, 22)).build();
      ...
      assertTrue(new ConflictManagementPredicate(api) {
         ...
         protected String operation() {
            return api().updateRole(roleName,
                  Role.create(role.roleName(), role.roleType(), role.vmImage(), role.mediaLocation(),
                        role.configurationSets(), role.resourceExtensionReferences(), role.availabilitySetName(),
                        role.dataVirtualHardDisks(), role.osVirtualHardDisk(), role.roleSize(),
                        role.provisionGuestAgent(), role.defaultWinRmCertificateThumbprint()));
         }
      }.apply(role.roleName()));
   }
